...
<div class="layui-layout layui-layout-admin">
    ...
    <div class="layui-body">
        ...
        <script>
            ...
            document.getElementById("d").innerHTML=d+" 部门下的新入职员工信息：";/*写入网页上方大字*/
        </script>
        <div id="tmp" class="app1"></div>
        <script>
            document.getElementById("tmp").innerHTML="       请输入入职时间范围进行查询";
        </script>
        ...
        <table id="tb_ne" lay-filter="tb_ne">
        </table>
...
<script>
    layui.use(['jquery','table'],function(){
        ...
        table.render({
            elem:"#tb_ne",//渲染的表格的id
            url:"${pageContext.request.contextPath}/getNewEList.action?dept_id="+ GetQueryValue("dept_id"),//请求后台的接口
            cols:[[//表头
...
<script>
    ...
    layui.use(['jquery', 'excel', 'layer'], function() {
        ...
        $('#exportExcel').on('click', function() {
            // 模拟从后端接口读取需要导出的数据
            $.ajax({
                url: "${pageContext.request.contextPath}/getNewEList.action?dept_id="+ GetQueryValue("dept_id")
                , dataType: 'json'
                , success(res) {
                    var data = res.data;
                    ...
                    excel.exportExcel(data, '新入职员工名单.xlsx', 'xlsx');
