...
<div class="layui-layout layui-layout-admin">
    ...
    <div class="layui-body">
        ...
        <script>
            ...
            document.getElementById("d").innerHTML=d+" 部门下的离职员工信息：";/*写入网页上方大字*/
        </script>
        <div id="tmp" class="app1"></div>
        <script>
            document.getElementById("tmp").innerHTML="       请输入离职时间范围进行查询";
        </script>
        ...
        <table id="tb_re" lay-filter="tb_re">
        </table>
...
<script>
    layui.use(['jquery','table'],function(){
        ...
        table.render({
            elem:"#tb_re",//渲染的表格的id
            url:"${pageContext.request.contextPath}/getResignedEList.action?dept_id="+ GetQueryValue("dept_id"),//请求后台的接口
            cols:[[//表头
                {field:'res_id', title:'离职员工编号', width:'120'},
                {field:'res_name', title:'姓名', width:'90'},
                ...
                {field:'job_id', title:'曾属岗位编号', width:'140'},
                {field:'resigned_time', title: '离职时间', width: '120'},
                {field:'resigned_reason', title: '离职原因', width: '260'}
            ]],
            page:false//是否开启分页
        });
...
<script>
    ...
    layui.use(['jquery', 'excel', 'layer'], function() {
        ...
        $('#exportExcel').on('click', function() {
            // 模拟从后端接口读取需要导出的数据
            $.ajax({
                url: "${pageContext.request.contextPath}/getResignedEList.action?dept_id="+ GetQueryValue("dept_id")
                , dataType: 'json'
                , success(res) {
                    var data = res.data;
                    ...
                    data.unshift({
                        ...
                        job_id: '岗位编号',
                        resigned_time: '离职时间',
                        resigned_reason: '离职原因'
                    });

                    var timestart = Date.now();
                    excel.exportExcel(data, '离职员工名单.xlsx', 'xlsx');
